...
function linkOptions(){return Array.from({length:6},(_,i)=>`<option value="L${i+1}">L${i+1}</option>`).join('')}
function renderTopologyInputRows(useExample=false){
 ...
 $('identifyGround').innerHTML=linkOptions();$('identifyInput').innerHTML=linkOptions();
 $('identifyGround').value=useExample?md().g:'L1';$('identifyInput').value=useExample?md().i:'L2';
 document.querySelectorAll('[data-topology-link]').forEach(x=>x.addEventListener('input',clearIdentification));
 $('identifyGround').onchange=clearIdentification;$('identifyInput').onchange=clearIdentification;
 clearIdentification();
}
...
function setTaskMode(mode){
 taskMode=mode;const identify=mode==='identify';
 $('identifyPanel').hidden=!identify;$('branchModelPanel').hidden=identify;
 $('taskIdentify').checked=identify;$('taskBranch').checked=!identify;
 $('taskIdentifyLabel').classList.toggle('active',identify);$('taskBranchLabel').classList.toggle('active',!identify);
 document.querySelectorAll('nav .nav').forEach((b,i)=>{if(i>0)b.disabled=identify;b.style.opacity=i>0&&identify?'.45':'1'});
 $('next').style.display=identify?'none':'';$('prev').style.display=identify?'none':'';
}
...
function identifyNineType(){
 ...
 if(errors.length){$('identifyStatus').innerHTML=errors.map(x=>`• ${x}`).join('<br>');$('identifyStatus').className='status bad';return}
...
function loadIdentifyExample(){renderTopologyInputRows(true)}
function applyIdentifiedModel(){if(!identifiedModel)return;selected=identifiedModel;init();renderCards();setTaskMode('branch');$('modelNote').insertAdjacentHTML('beforeend','<br><b>已由使用者輸入的拓樸辨識結果自動套用。</b>')}
